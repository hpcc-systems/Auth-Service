...
function ApplicationsList() {
  ...
  const [notificationSettingsConfigured, setNotificationSettingsConfigured] = useState(false);

  //Check if notification is configured in backend
  const checkIfNotificationSettingsConfigured = async () => {
    try {
      const rawResponse = await fetch("/api/notification/notificationSettings", { headers: authHeader(), method: "get" });
      const response = await rawResponse.json();
      console.log(response);

      if (response.notificationSettingConfigured) {
        setNotificationSettingsConfigured(true);
      }
    } catch (err) {
      console.log(err);
    }
  ...
  const columns = [
    {
      ...
      render: (text, record) => (
        <span className="link-style" onClick={(row) => handleEdit(record)}>
          {text}
        </span>
      ),
    },
    ...
    <React.Fragment>
      <span>
        ...
        <Tooltip title="Add Application">
          <Button className="add-button" type="primary" icon={<FileAddOutlined />} onClick={addApplication}>
            Add
          </Button>
        </Tooltip>
      </span>
