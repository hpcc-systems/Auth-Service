        ...
        .then(async (data) => {
          ...
          if (data) {
            if (
              data[0] &&
              data[0].permissions &&
              data[0].permissions.length > 0
            ) {
              Object.keys(data[0].permissions).forEach((permissionKey, idx) => {
                data[0].permissions[permissionKey][
                  Object.keys(data[0].permissions[permissionKey])[0]
                ].forEach((permission) => {
                  console.log(JSON.stringify(permission));
                  console.log(permission.field_type);
                  if (permission.field_type === "radio") {
                    let defaultValue = permission.ui_values.filter(
                      (uiValue) => uiValue.default === true
                    );
                    if (
                      roleDetails.permissions &&
                      roleDetails.permissions[permission.key]
                    ) {
                      initialValues.push({
                        [permission.key]:
                          roleDetails.permissions[permission.key],
                      ...
                      initialValues.push({
                        [permission.key]: defaultValue[0].value,
                      });
                    }
                  } else {
                    if (
  ...
  return (
    <React.Fragment>
      <Breadcrumb className="bread-crumb">
        ...
        <Breadcrumb.Item>
          {roleDetails ? roleDetails.name : "New Role"}
        </Breadcrumb.Item>
      </Breadcrumb>
      {data && data.length > 0 ? (
            ...
            <TabPane tab={"Role Info"} key={"roleInfo"} className="tab-content">
              ...
              <Form.Item
                ...
                rules={[
                  {
                    required: true,
                    pattern: new RegExp(/^[a-zA-Z]{1}[a-zA-Z0-9 _-]*$/),
                ...
                rules={[
                  {
                    required: true,
                    pattern: new RegExp(/^[a-zA-Z]{1}[a-zA-Z0-9@ ._-]*$/),
                    message: "Please enter managed by information!",
                  },
                ]}
              >
                <Input style={{ width: "400px" }} />
            ...
            {data[0].permissions.map((permissionCategory, catIdx) =>
              Object.keys(permissionCategory).map((category, idx) => (
                <TabPane
                  ...
                  forceRender={true}
                >
                  {data[0].permissions[catIdx][category].map(
                    (permissionType) => (
                      <React.Fragment>
                        <Tooltip
                          placement="rightTop"
                          ...
                          {permissionType.field_type === "radio" ? (
                            <Radio.Group name={permissionType.key}>
                              {permissionType.ui_values.map((uiValue) => (
                                <Tooltip
                                  placement="rightTop"
                                  key={uiValue.displayValue}
                                  title={uiValue.description}
                                >
                                  <Radio value={uiValue.value}>
